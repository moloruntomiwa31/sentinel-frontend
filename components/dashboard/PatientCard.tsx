		...
		>
			<div className="flex items-center justify-between mb-4">
				<div>
					...
					>
						<h3 className="font-bold text-lg cursor-pointer">{patient.name}</h3>
					</Link>
			...
			<div className="h-16 mb-4 flex items-end gap-1">
				{(patient.trend_data || []).map((value, index) => (
					<div
						...
						style={{
							height: `${(value / Math.max(...(patient.trend_data || [1]))) * 100}%`,
						}}
					/>
				))}
			</div>
				...
				<div className="flex items-center gap-2 mb-2">
					...
					<span className="text-sm">
						{patient.risk_level === "STABLE"
							...
							: patient.risk_level === "ELEVATED RISK"
								? "Upward trend in Heart Rate noted."
